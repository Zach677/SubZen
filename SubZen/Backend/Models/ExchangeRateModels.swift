...
struct ExchangeRateResponse: Codable {
  ...
  var isSuccess: Bool {
    return result == "success" || result == nil  // 免费API可能没有result字段
  }
}
...
struct ExchangeRateConfig {
  ...
  enum Endpoint {
    ...
    var path: String {
      switch self {
      case .latest(let baseCurrency):
        return "/latest/\(baseCurrency)"
      case .pair(let from, let to, let amount):
        if let amount = amount {
          return "/pair/\(from)/\(to)/\(amount)"
        } else {
          return "/pair/\(from)/\(to)"
        }
      case .historical(let baseCurrency, let year, let month, let day):
        return "/history/\(baseCurrency)/\(year)/\(month)/\(day)"
      }
    }

    var fullURL: String {
      return "\(ExchangeRateConfig.baseURL)/\(ExchangeRateConfig.apiKey)\(path)"
    }
  }
}
