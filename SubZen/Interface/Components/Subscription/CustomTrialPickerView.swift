...
final class CustomTrialPickerView: UIView {
    private enum Layout {
    ...
    private let forLabel = UILabel().with {
        $0.font = UIFont.systemFont(ofSize: 15, weight: .medium)
        $0.textColor = .secondaryLabel
        $0.text = String(localized: "For")
    }

    private let valueRange = Array(1 ... 99)
    private let units: [CycleUnit] = [.day, .week, .month]

    private var selectedValue: Int = 7
    private var selectedUnit: CycleUnit = .day
    ...
    private func configureView() {
        ...
        // Set initial selection: 7 days
        valuePicker.selectRow(6, inComponent: 0, animated: false) // "7"
        unitPicker.selectRow(0, inComponent: 0, animated: false) // "day"
    }

    private func buildHierarchy() {
        addSubview(containerView)
        containerView.addSubview(forLabel)
    ...
    private func setupConstraints() {
        ...
        valuePicker.snp.makeConstraints { make in
            make.top.equalTo(forLabel.snp.bottom).offset(-8)
...
extension CustomTrialPickerView: UIPickerViewDataSource, UIPickerViewDelegate {
